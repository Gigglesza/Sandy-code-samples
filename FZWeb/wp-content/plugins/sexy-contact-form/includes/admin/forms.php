...
if($task == 'unpublish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."sexy_forms SET `published` = '0' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'publish') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "UPDATE ".$wpdb->prefix."sexy_forms SET `published` = '1' WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
if($task == 'delete') {
	if(is_array($ids)) {
		foreach($ids as $id) {
			$idk = (int)$id;
			if($idk != 0) {
				$sql = "DELETE FROM ".$wpdb->prefix."sexy_forms WHERE `id` = '".$idk."'";
				$wpdb->query($sql);
			}
		}
	}
...
?>
<form action="admin.php?page=sexyforms" method="post" id="wpscf_form">
<div style="overflow: hidden;margin: 0 0 10px 0;">
	<div style="float: left;">
		...
		<a href="admin.php?page=sexyforms"  class="button">Reset</a>
	</div>
	<div style="float:right;">
		<a href="admin.php?page=sexyforms&act=new" id="wpscf_add" class="button-primary">New</a>
...
<table class="widefat">
	<thead>
		<tr>
			...
			<th nowrap align="center" style="width: 50px;text-align: center;">Fields</th>
			<th nowrap align="center" style="width: 30px;text-align: center;">Id</th>
		</tr>
	</thead>
<tbody id="wpscf_sortable" table_name="<?php echo $wpdb->prefix;?>sexy_forms" reorder_type="reorder">
<?php        
			$k = 0;
